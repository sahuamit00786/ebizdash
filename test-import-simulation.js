...
// Test data that simulates the CSV import
const testData = [
  {
  ...
  {
    vendor_category: 'Electronics',
    vendor_subcategory_1: 'Laptops',
    vendor_subcategory_2: 'Windows',
    vendor_subcategory_3: 'Gaming',
    vendor_subcategory_4: 'High Performance',
    sku: 'LAPTOP001',
    name: 'Alienware X17'
  }
];
...
async function processCategoryHierarchy(row, mapping, type, categoryCache, connection) {
  try {
    ...
    for (const subcategory of subcategories) {
      // Create cache key for this subcategory level
...
async function testHierarchicalImport() {
  ...
  try {
    ...
    for (let i = 0; i < testData.length; i++) {
      ...
      console.log(`Processing row ${i + 1}: ${row.sku} - ${row.name}`);
...
module.exports = { testHierarchicalImport };
